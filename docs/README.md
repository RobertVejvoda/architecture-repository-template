...
This page tracks the version of the template document structure itself. Project-specific baseline, target, and transition architecture versions should be tracked in [Architecture Version Register](/architecture/architecture-states/architecture-version-register.md). Project-specific repository-structure changes should be tracked in [Repository Changes](/architecture/repository-changes.md).
...
| 0.6 | 2026-05-31 | Draft | Replaced thin placeholder rows with representative sample rows across architecture artifacts. | Template users should replace sample rows with project-specific content while preserving the traceability pattern. |
...
| --- | --- | --- |
| New reader | [Architecture In Plain Words](/architecture/plain-language-introduction.md) | Understand the concepts and how to work with the repository. |
