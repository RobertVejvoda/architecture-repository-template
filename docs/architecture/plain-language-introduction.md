...
It is not a project plan, not a task board, and not a place to manage daily work. Use the delivery tool for implementation tasks. This repository answers different questions:
...
Put it in [Candidate Architectures](/architecture/architecture-states/candidate-architectures.md).
...
When the idea is accepted, it can move into [Target Architecture](/architecture/architecture-states/target-architecture.md) or [Transition Architectures](/architecture/architecture-states/transition-architectures.md).

## What Happens When A New Requirement Appears?

Start with [Requirements](/architecture/requirements.md).
...
If the answer affects several areas, use [Architecture Change Set](/architecture/governance/architecture-change-set.md) as a checklist so the update is not made in one page while other pages stay inconsistent.

## What Happens When Governance Has A Question?

Use [Architecture Governance Log](/architecture/governance/architecture-governance-log.md) only while the question is active.

The log is not permanent history. Once the question is answered:

- a decision goes to [Decisions](/architecture/decisions.md);
- a gap goes to [Gap Analysis](/architecture/architecture-states/gap-analysis.md);
- a risk goes to [Risk Register](/architecture/architecture-states/risk-register.md);
- an exception goes to [Waivers](/architecture/governance/waivers.md);
- delivery work goes to the delivery tool;
...
Use [Architecture Version Register](/architecture/architecture-states/architecture-version-register.md) to see what belongs together.

Use [Artifact Register](/architecture/artifact-register.md) to see the status and owner of each artifact.
...
- delivery tasks in the delivery tool.
